...
const renderLink = (href, label, variant) => {
  const baseClasses =
    "inline-flex items-center justify-center rounded-full px-5 py-3 text-xs font-semibold uppercase tracking-[0.24em] transition-all duration-300";

  const styles = {
    primary:
      "bg-gradient-to-r from-violet-500 via-fuchsia-500 to-cyan-400 text-slate-950 shadow-xl shadow-violet-500/20 hover:-translate-y-2 hover:scale-105",
    secondary:
      "border border-white/15 bg-white/[0.04] text-white shadow-lg shadow-black/20 hover:-translate-y-2 hover:scale-105 hover:border-cyan-400/40 hover:bg-white/[0.08]",
    disabled:
...
const Projects = () => {
  return (
    <section
      id="projects"
      className="relative overflow-hidden bg-[#17191d] px-6 py-20 text-white sm:px-8 lg:px-12 lg:py-24"
    >
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-5 text-base leading-8 text-gray-400 sm:text-xl">
        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-2">
          {projects.map((project, projectIndex) => {
            ...
            return (
              <article
                key={project.title}
                className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.03] p-6 shadow-2xl shadow-black/40 backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 hover:scale-[1.01] hover:border-white/20 sm:p-8"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-transparent opacity-70" />
                <div className="absolute -right-16 top-8 h-40 w-40 rounded-full bg-violet-500/10 blur-3xl transition-all duration-300 group-hover:bg-violet-500/20" />
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    {projectImages.map((image, imageIndex) => (
                      <div
                        ...
                        className="relative aspect-[4/3] overflow-hidden rounded-[1.35rem] border border-white/10 bg-gradient-to-br from-zinc-900 via-zinc-950 to-black shadow-xl shadow-black/40 transition-all duration-300 group-hover:-translate-y-1"
                      >
                        {image.src ? (
                          <img
                            src={image.src}
